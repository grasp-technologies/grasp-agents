...
async def test_palette_opens_on_slash(tmp_path: Path) -> None:
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        palette = app.query_one("#skill-palette", SkillPalette)
        assert palette.display is False
        app.query_one("#prompt", PromptArea).insert("/")
        await pilot.pause()
...
async def test_palette_filters_by_name(tmp_path: Path) -> None:
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        app.query_one("#prompt", PromptArea).insert("/bra")
        await pilot.pause()
        palette = app.query_one("#skill-palette", SkillPalette)
        assert palette.display is True
...
async def test_non_command_text_keeps_palette_hidden(tmp_path: Path) -> None:
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        app.query_one("#prompt", PromptArea).insert("hello there")
        await pilot.pause()
        assert app.query_one("#skill-palette", SkillPalette).display is False
...
async def test_palette_enter_inserts_command_without_submitting(
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        assert app.query_one("#skill-palette", SkillPalette).display is True
...
async def test_escape_closes_palette(tmp_path: Path) -> None:
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        app.query_one("#prompt", PromptArea).insert("/pro")
        await pilot.pause()
        assert app.query_one("#skill-palette", SkillPalette).display is True
...
def test_example_skills_load() -> None:
    from grasp_agents.examples.tui.skills_copilot import _SKILLS_ROOT
...
def test_example_build_copilot_constructs() -> None:
    from grasp_agents.examples.tui.skills_copilot import build_copilot
