...
# ==== WebSearchCallItem with ActionOpenPage ====
...
def _make_oai_response_with_web_fetch() -> OAIResponse:
    """Build a minimal OpenAI Response containing a web_search_call with ActionOpenPage."""
...
class TestWebFetchRoundtrip:
    """WebSearchCallItem(ActionOpenPage) survives model_dump/model_validate."""

    def test_response_to_internal(self) -> None:
        """OAI Response with ActionOpenPage → InternalResponse preserves action type."""
        ...
        assert isinstance(open_page.action, ActionOpenPage)
        ...
        assert search.action.query == "example query"

    def test_sanitize_strips_provider_fields(self) -> None:
        """_sanitize_for_api removes provider_specific_fields from items."""
        ...
        sanitized = _sanitize_for_api([item])
    ...
    def test_failed_status_preserved(self) -> None:
        ...
        assert isinstance(ws_items[0].action, ActionOpenPage)
