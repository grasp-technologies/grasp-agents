...
Uses the test MCP server at tests/mcp/mcp_test_server.py via stdio transport.
"""
...
from grasp_agents.mcp.client import MCPClient, MCPServerSSE, MCPServerStdio
...
# ---------- omitted optionals are not sent as null ----------
...
# ---------- file backend: ".." collapse + same-surface mtimes ----------
...
def test_mcp_server_sse_config() -> None:
    server = MCPServerSSE(url="https://mcp.example.test/sse")
    assert server.url == "https://mcp.example.test/sse"
    # Construction wires the config without connecting (no network).
    client = MCPClient("remote", server=server)
