...
def build_srt_settings(policy: SandboxPolicy) -> dict[str, object]:
    ...
    ``network.allowLocalBinding`` is always on so a co-located Jupyter kernel can
    bind its loopback ZMQ ports (loopback only — external egress stays
    allowlist-gated by srt's proxy). srt manages its own writable temp dir for
    the sandboxed process, so no extra ``allowWrite`` entry is needed.
    """
