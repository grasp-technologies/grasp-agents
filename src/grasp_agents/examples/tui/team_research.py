...
def build_member(
    name: str, *, mailbox_dir: Path = DEFAULT_DIR
) -> tuple[MemberHost, Processor[Any, Any, None]]:
    """
    ...
    ``FileCheckpointStore`` under ``mailbox_dir``. Returns the host and the
    member itself (the TUI takes the member to restore its pane on relaunch).
    """
    ...
    return MemberHost(member, cards=ROSTER), member


def build_team(
    *, mailbox_dir: Path = DEFAULT_DIR
) -> tuple[AgentTeam[None], list[Processor[Any, Any, None]]]:
    """
    ...
    team's hop budget all persist to the same store. Returns the team and its
    members (the TUI takes the members to restore their panes on relaunch).
    """
    mailbox_dir.mkdir(parents=True, exist_ok=True)
    ctx = _make_session(mailbox_dir)
    members = [
    ...
    return AgentTeam(members, entry="lead", cards=ROSTER, ctx=ctx), members
...
def main() -> None:
    ...
    if args.who == "team":
        ...
        # in-flight turns) and the next message can start a fresh one. Passing
        # the member objects (not just names) pre-creates every pane AND lets a
        # relaunch over the same --dir restore each member's full state —
        # initial context, transcript, task logs, queued mail.
        team, members = build_team(mailbox_dir=args.dir)
    ...
    host, member = build_member(args.who, mailbox_dir=args.dir)
    run_tui_interactive(
