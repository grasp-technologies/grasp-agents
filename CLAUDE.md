...
|-----------|----------|---------|
| `LLMAgent` | `agent/llm_agent.py` | Core agent: LLM + tools + `AgentLoop`. Owns the session lifecycle: shells/kernels/background tasks survive run boundaries and are released only by `await agent.aclose()` (or `async with agent:`), cascading to wrapped sub-processors |
| `AgentLoop` | `agent/agent_loop.py` | The agentic loop with RL-style PRE-ACT/ACT/JUDGE/OBSERVE phases; enforces `max_turns`, injects dangling-tool-call cancellations on exhaustion, integrates `BackgroundTaskManager`. A pure per-run executor — no resource teardown at run end |
...
| `AgentContext` | `agent/agent_context.py` | Per-agent, session-scoped mutable state (transcript, tools, file-edit state, bg tasks, kernel/session holders), passed to every tool call — the seam that keeps tools stateless. |
...
| `RunContext[CtxT]` | `run_context.py` | Session-scoped DI container (user state, usage tracker, printer, stores). Bare-built processors bind to the ambient `with RunContext(...)` ctx or a process-wide default at construction — never a per-agent throwaway; `current_run_context()` resolves it |
...
- **One session per `RunContext`, bound at construction**: a processor built without an explicit `ctx=` binds — **once, in `__init__`** — to the ambient ctx of any enclosing `with RunContext(...)` block, else to a lazily-created process-wide default (`current_run_context()` in `run_context.py`). There is no per-agent throwaway ctx: two bare agents that are never composed still share one session (usage tracker / `state` / stores). Because binding is construction-time and the default is process-wide, **multi-tenant servers must scope construction per request** — either build agents inside `with RunContext(per_request):`, or rebind a bare-built agent with `on_adopted(ctx=...)` — otherwise sibling requests share the one default ctx. `ContextVar` makes the ambient binding task-local, so concurrent sessions in one process stay isolated. **ctx flows strictly top-down**: a container processor (`Runner`/workflows/parallel) resolves its own ctx (explicit or ambient/default) and cascades it onto every subprocessor via `on_adopted`, overriding whatever they were built with — a subprocessor never carries its own session (same as `ProcessorTool`/`AgentTool` rebinding a wrapped processor to the call's ctx). So **never pass `ctx=` to a subprocessor**; set it on the container or via `with RunContext(...)`. `RunContext.__deepcopy__` returns `self`, so replicas/clones never fork the session.
